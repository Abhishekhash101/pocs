...
modulations = st.multiselect("Select Modulations",
    ['AM','DSB-SC','SSB','FM'])
...
if uploaded_file is not None and modulations:
    ...
    if input_type=="Audio":
        ...
        st.audio(msg, format='audio/wav', sample_rate=fs)
        ...
        # Show waveform of original image
    ...
    for mod in modulations:
        ...
        # Side-by-side plots for better comparison
        ...
        # Display audio or image
        if input_type=="Audio":
            st.audio(rec, format='audio/wav', sample_rate=fs)
        else:
            img_rec = recover_image(rec, shape)
            st.image(img_rec, caption=f"{mod} Recovered Image", use_column_width=True)
        st.markdown("<br><br>", unsafe_allow_html=True)
